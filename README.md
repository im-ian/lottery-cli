...
| `autoAllPensionAllGroups` | `false` | `모두 자동 구매`에서 연금복권을 모든 조로 구매 |
| `autoAllPensionAllGroupsMinDeposit` | `25000` | 위 옵션을 자동 활성화할 최소 예치금. 조건 미달 시 일반 연금 자동 5게임으로 대체 |
...
- 예치금 충전은 CLI가 고정 가상계좌 발급까지 진행합니다. 실제 송금은 은행 앱에서 직접 처리해야 합니다.
- 연금복권 `자동 모든 조`는 같은 번호를 1~5조로 구매합니다. `모두 자동 구매`에서는 설정이 켜져 있어도 최소 예치금/구매 예정 금액 조건을 만족하지 않으면 자동으로 일반 연금 자동 구매로 대체합니다.
- 연금복권은 5게임을 초과해 입력하면 5게임 단위로 나누어 연속 구매합니다.
- 로또 6/45는 한 회차 최대 5게임 제한이 있어 연속 구매로 초과 구매하지 않습니다.
